...
public class HungarianAlgorithmEdu {
																				...
																				// headers.
	{
		...
		System.out.print(stars + "\n" + "*" + stepTitle + "*" + "\n" + stars
				+ "\n");
		HungarianAlgorithmEdu.insertLines(blancLinesAfter);
	}
	...
	(double[][] array, String elementFormatter, String title,
		...
		for (double[] element : array) {
			for (int j = 0; j < element.length; j++) {
				System.out.printf(elementFormatter, element[j]);
			}
			// System.out.println();
		}
		HungarianAlgorithmEdu.insertLines(blancLinesAfter);
	...
	public static int[][] hgAlgorithm(double[][] array, String sumType) {
		double[][] cost = HungarianAlgorithmEdu.copyOf(array); // Create the
																// cost matrix.
		...
		if (sumType.equalsIgnoreCase("max")) // Then array is weight array. Must
												// change to cost.
		{
			double maxWeight = HungarianAlgorithmEdu.findLargest(cost);
			...
			HungarianAlgorithmEdu.print2DArray(cost, "%.1f\t",
					"The cost matrix (generated from weight matrix) is:", 1);
		}

		double maxCost = HungarianAlgorithmEdu.findLargest(cost); // Find
																	// largest
																	// cost
																	// matrix
																	// element
		...
		HungarianAlgorithmEdu.printStepHeader(
				"**** HUNGARIAN ALGORITHM STARTING *****", 1);

		// Create the main execution loop
		while (done == false) {
			switch (step) {
			case 1:
				step = HungarianAlgorithmEdu.hg_step1(step, cost);
				break;

			case 2:
				step = HungarianAlgorithmEdu.hg_step2(step, cost, mask,
						rowCover, colCover);
				break;

			case 3:
				step = HungarianAlgorithmEdu.hg_step3(step, mask, colCover);
				break;

			case 4:
				step = HungarianAlgorithmEdu.hg_step4(step, cost, mask,
						rowCover, colCover, zero_RC);
				break;

			case 5:
				step = HungarianAlgorithmEdu.hg_step5(step, mask, rowCover,
						colCover, zero_RC);
				break;

			case 6:
				step = HungarianAlgorithmEdu.hg_step6(step, cost, rowCover,
						colCover, maxCost);
		...
		// System.out.println
		// ("Algorithm out of main loop.\nProcess exited with code " + step +
		// ".\n");
		HungarianAlgorithmEdu.printStepHeader(
				"**** HUNGARIAN ALGORITHM FINISHED! *****", 1);
	...
	public static int hg_step1(int step, double[][] cost) {
		...
		HungarianAlgorithmEdu.printStepHeader("Step 1 (executed only once)", 1);
		...
		HungarianAlgorithmEdu.print2DArray(cost, "%.1f\t",
				"The cost matrix before step 1 is:", 1);
	...
	public static int hg_step2(int step, double[][] cost, int[][] mask,
		...
		HungarianAlgorithmEdu.printStepHeader("Step 2 (executed only once)", 1);
		System.out.println("Step 2 examines the cost matrix and sets\n"
				...
				+ "zeros are found.\n");
		HungarianAlgorithmEdu.print2DArray(cost, "%.1f\t",
				"Cost matrix passed to step 2:", 0);
		HungarianAlgorithmEdu.print1DArrayInt(rowCover, "%d   ",
				"Row cover matrix passed to step 2 is:", 1);
		HungarianAlgorithmEdu.print1DArrayInt(colCover, "%d   ",
		...
		for (int i = 0; i < cost.length; i++) {
			for (int j = 0; j < cost[i].length; j++) {
				if (cost[i][j] == 0 && colCover[j] == 0 && rowCover[i] == 0) {
					...
					colCover[j] = 1;
					rowCover[i] = 1;
					HungarianAlgorithmEdu.print2DArrayInt(mask, "%d\t",
		...
		HungarianAlgorithmEdu.clearCovers(rowCover, colCover);
		System.out.println("Did it.");
		HungarianAlgorithmEdu.print1DArrayInt(rowCover, "%d   ",
				"Row cover matrix resetted:", 1);
		HungarianAlgorithmEdu.print1DArrayInt(colCover, "%d   ",
				"Col cover matrix resetted:", 2);
	...
	public static int hg_step3(int step, int[][] mask, int[] colCover) {
		...
		HungarianAlgorithmEdu.printStepHeader("Step 3", 1);
		...
		HungarianAlgorithmEdu.print2DArrayInt(mask, "%d\t",
				"Mask matrix passed to step 3:", 1);
	...
	public static int hg_step4(int step, double[][] cost, int[][] mask,
		...
		boolean done = false;
		while (done == false) {
			row_col = HungarianAlgorithmEdu.findUncoveredZero(row_col, cost,
					rowCover, colCover);
			if (row_col[0] == -1) {
	...
	(int[] row_col, double[][] cost, int[] rowCover, int[] colCover) {
		...
		HungarianAlgorithmEdu.print2DArray(cost, "%.1f\t",
				"Cost matrix is (for easy reference):", 1);
	...
	public static int hg_step5(int step, int[][] mask, int[] rowCover,
		...
		HungarianAlgorithmEdu.printStepHeader("Step 5", 1);
		...
		HungarianAlgorithmEdu.print2DArrayInt(mask, "%d\t",
				"Mask matrix passed to step 5:", 0);
		HungarianAlgorithmEdu.printPath(path, count + 1, "%d\t",
				"The path matrix, therefore, becomes: ", 2);

		boolean done = false;
		while (done == false) {
			int r = HungarianAlgorithmEdu.findStarInCol(mask, path[count][1]);
			if (r >= 0) {
				count = count + 1;
			...
			if (done == false) {
				int c = HungarianAlgorithmEdu.findPrimeInRow(mask,
						path[count][0]);
				count = count + 1;
		...
		HungarianAlgorithmEdu.convertPath(mask, path, count);

		// System.out.println("The Cover arrays are being cleared...");
		HungarianAlgorithmEdu.clearCovers(rowCover, colCover);
		// System.out.println("Did it.");
		HungarianAlgorithmEdu.print1DArrayInt(rowCover, "%d   ",
				"Row cover matrix cleared:", 1);
		HungarianAlgorithmEdu.print1DArrayInt(colCover, "%d   ",
				"Col cover matrix cleared:", 2);

		HungarianAlgorithmEdu.erasePrimes(mask);
		HungarianAlgorithmEdu.print2DArrayInt(mask, "%d\t",
	...
	(int[][] mask, int col) {
		...
		HungarianAlgorithmEdu.print2DArrayInt(mask, "%d\t",
				"Mask matrix (for easy reference):", 0);
	...
	(int[][] mask, int[][] path, int count) {
		...
		HungarianAlgorithmEdu.print2DArrayInt(mask, "%d\t",
				"Mask matrix before convertPath():", 0);
		...
		HungarianAlgorithmEdu.print2DArrayInt(mask, "%d\t",
				"Mask matrix after convertPath()", 1);
	}
	...
	public static int hg_step6(int step, double[][] cost, int[] rowCover,
		...
		HungarianAlgorithmEdu.printStepHeader("Step 6", 1);
		...
		HungarianAlgorithmEdu.print2DArray(cost, "%.1f\t",
				"Cost matrix passed to step 6:", 0);
		HungarianAlgorithmEdu.print1DArrayInt(rowCover, "%d   ",
		...
		double minval = 0;
		minval = HungarianAlgorithmEdu.findSmallest(cost, rowCover, colCover,
				maxCost);
	...
	public static void main(String[] args) {
		...
		double[][] array = { { 14, 81, 78 }, { 54, 95, 28 }, { 67, 33, 51 } };
		 ...
		 * because of the crowded output. int numOfRows =
		...
		if (array.length > array[0].length) {
			...
			array = HungarianAlgorithmEdu.transpose(array);
		}

		HungarianAlgorithmEdu.print2DArray(array, "%.1f\t",
				"The inputted matrix is:", 1);

		double startTime = System.nanoTime();

		int[][] assignment = new int[array.length][2];
		assignment = HungarianAlgorithmEdu.hgAlgorithm(array, sumType); // Call
																		// Hungarian
																		// algorithm.
		...
		HungarianAlgorithmEdu.print2DArray(array, "%.1f\t", "The initial "
				+ arrayType + " array was:", 1);
		...
		for (int[] element : assignment) {
			...
			System.out.printf("array(%d,%d) = %.1f\n", element[0] + 1,
					element[1] + 1, array[element[0]][element[1]]);
